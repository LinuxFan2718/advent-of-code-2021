...
while True:
  ...
  if large_y >= len(vents[0]):
    zeroes_to_add = [0] * (large_y - len(vents[0]) + 2)
  ...
  if x1 == x2:
    ...
    # make list of pairs comprising this diagonal
    pairs = []
    ...
    if x1 < x2:
      ...
      left_point = [x2, y2]
      right_point = [x1, y1]
    if left_point[1] < right_point[1]: # down and right diagonal
      xs = list(range(left_point[0], right_point[0] + 1))
      ys = list(range(left_point[1], right_point[1] + 1))
      pairs = zip(xs, ys)
    elif left_point[1] > right_point[1]: # up and right diagonal
      xs = list(range(left_point[0], right_point[0] + 1))
      ys = list(range(left_point[1], right_point[1] - 1, -1))
      pairs = zip(xs, ys)
    for pair in pairs:
      vents[pair[0]][pair[1]] += 1
